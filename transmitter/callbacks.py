    ...
    if values[Keys.TOGGLE_PLAIN_TEXT]:
        ...
        if len(failed_files) != 0:
            window[Keys.FILES_PATH].update(
                row_colors=list(zip(failed_files_ind, ["red"] * len(failed_files_ind)))
            )
            sg.popup(
...
def remove_files(window, values):
    ...
    current_files = window[Keys.FILES_PATH].Values
    final_files = [
        file for [ind, file] in enumerate(current_files) if ind not in files_to_remove
    ]
...
def move_file_callback_generator(isMoveUp: bool):
    ...
    def move_file_callback(window, values):
        ...
        for i in iterator:
            ...
            if (
                ind_to_swap_with >= 0
                and ind_to_swap_with < len(all_files)
                and not ind_to_swap_with in highlight_indexes
            ):
